...
export function QuizResultGentle({
  primaryProfile,
  secondaryProfile,
  articles,
  onRestart,
}: QuizResultGentleProps) {
  return (
    <section className="rounded-[24px] border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-[0_16px_34px_rgba(36,76,99,0.08)] lg:p-8">
      <p className="text-sm text-[var(--color-text-muted)]">Dựa trên những điều bạn vừa chọn, có vẻ lúc này...</p>
      <h3 className="mt-2 text-[clamp(1.5rem,2.5vw,2.1rem)] font-semibold leading-[1.25] text-[var(--color-text-strong)]">
      ...
      <div className="mt-6 grid gap-3 lg:grid-cols-2">
        <div className="rounded-[18px] border border-[var(--color-border)] bg-[rgba(217,228,218,0.5)] p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.08em] text-[var(--color-teal)]">Có thể nên đọc trước</p>
      ...
      <div id="goi-y-bai-viet" className="mt-7 space-y-3">
        <p className="text-sm font-semibold uppercase tracking-[0.08em] text-[var(--color-teal)]">Gợi ý để bắt đầu</p>
        <ul className="grid gap-3">
          {articles.map((article) => (
            <li key={article.href} className="rounded-[16px] border border-[var(--color-border)] bg-[rgba(251,248,242,0.92)] p-4">
              <Link
                href={article.href}
      ...
      <div className="mt-7 flex flex-wrap gap-3">
        <Link
          ...
          className="calm-action calm-action--primary inline-flex items-center justify-center rounded-full bg-[var(--color-navy)] px-5 py-2.5 text-sm font-semibold text-[#F6F1E8] shadow-[0_12px_26px_rgba(36,76,99,0.20)] transition-all duration-200 hover:bg-[#1f4256] hover:shadow-[0_16px_30px_rgba(36,76,99,0.28)]"
        >
          Đọc phần này trước
        </Link>
        <a
