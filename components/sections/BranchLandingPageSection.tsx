...
import type { ContentBranchLanding } from "@/data/contentBranches";

type BranchLandingPageSectionProps = {
  content: ContentBranchLanding;
};

export function BranchLandingPageSection({ content }: BranchLandingPageSectionProps) {
  return (
          ...
          <div className="rounded-[24px] border border-[var(--color-border)] bg-[var(--color-card)] p-6 shadow-[0_16px_34px_rgba(36,76,99,0.08)] lg:p-8">
            ...
            <div className="mt-7 grid gap-4 lg:grid-cols-3">
              {content.clusters.map((cluster) => (
                ...
                >
                  <h2 className="text-lg font-semibold leading-[1.32] text-[var(--color-text-strong)]">{cluster.title}</h2>
                  <p className="mt-2 text-sm text-[var(--color-text-muted)]">{cluster.summary}</p>
                  <Link
                    href={cluster.href}
            ...
            <section className="mt-8 rounded-[20px] border border-[var(--color-border)] bg-[rgba(245,239,230,0.62)] p-5 lg:p-6">
              <h2 className="text-[1.16rem] font-semibold leading-[1.35] text-[var(--color-text-strong)]">Đọc tiếp theo hướng liên quan</h2>
